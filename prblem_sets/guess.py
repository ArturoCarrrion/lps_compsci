...
#the next block is a if stament wich checks if its true or false so it checks if the number that the user enters is greater than the number that the computer chose. Also it lets the user know if there number is to high.
	if number > num:
 		print('to hight please enter another number:')
#the next block is a elif stament wich checks if its true or false if the line above does not apply to it so it checks if the number that the user enters is less than the number that the computer chose and lets the user know if the number they enter is to low.  
	elif number < num:
